...
! ***********************************************************************
      ...
      subroutine fixed_energy_dep(id, ierr)
         implicit none
         ...
         type (star_info), pointer :: s
         integer                   :: kcut, nz
         integer, parameter :: maxzones = 100000
         real(dp) :: sigma, lumdep, rdep, norm, smooth, rcut
         real(dp), dimension(maxzones) :: dr!, dm
         ...
         sigma = 0.25*rsun !0.25 * rsun
         lumdep = s% x_ctrl(1)*s% prev_Ledd ! Total luminosity deposited (erg/s)
         rdep = s% x_ctrl(2) * rsun ! Radius of deposition
         rcut = 1d-1 * rsun ! Innermost radius with energy deposition


         s% extra_heat(:) = 0.
         dr(nz) = 0.
         dr(1:nz - 1) = s% r(1:nz - 1) - s% r(2:nz)

         smooth = tanh(s% star_age)
         kcut = minloc(abs(s% r(1:nz) - rcut),dim=1)
         norm = lumdep / (sigma * sqrt(2d0 * pi))
         !dm(1:kcut) = s% star_mass * msun * s% dq(1:kcut) ! Mass of shell
         ! Deposit luminosity as Gaussian
         s% extra_heat(1:kcut) = norm * exp(-(s% r(1:kcut) - rdep) ** 2d0 &
            / (2d0 * sigma ** 2d0)) / s% dm(1:kcut) * dr(1:kcut) * smooth

      end subroutine fixed_energy_dep
